    ...
    def _analyze_sentiment_with_ollama(self,
        ...
        try:
            ...
            if analysis_type == "detailed":
                prompt = f"以下のテキストの感情を詳細に分析してください。以下の形式で回答してください: 全体的な感情: [ポジティブ/ネガティブ/ニュートラル] 感情の強さ: [弱い/中程度/強い] 主要な感情: [喜び/悲しみ/怒り/恐れ/驚き/嫌悪/その他] トーン: [フォーマル/カジュアル/情熱的/冷静/その他] テキスト: 「{content}」"
            elif analysis_type == "emotional":
                prompt = f"以下のテキストに含まれる感情を分析し、感情ごとの強度を評価してください。喜び: [0-10の数値] 悲しみ: [0-10の数値] 怒り: [0-10の数値] 恐れ: [0-10の数値] 驚き: [0-10の数値] 全体的印象: [一言で] テキスト: 「{content}」"
            else:  # basic
                prompt = f"以下のテキストの感情を分析してください。「ポジティブ」「ネガティブ」「ニュートラル」のいずれかで回答し、その後に理由を簡潔に説明してください。テキスト: 「{content}」"
